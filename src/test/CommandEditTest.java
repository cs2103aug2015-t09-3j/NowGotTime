...
public class CommandEditTest extends CommandTest {
    ...
    public void testCanEditEvent() throws Exception {
        ...
        new CommandEditItem(name, CommonHelper.FIELD_START, "19 Jan 2015").execute(service, project, history);
        event = service.viewSpecificEvent(name);
        assertEquals(event.getStartDateTimeString(), "19 Jan 2015 10:00");
        
        // test can edit start time
        new CommandEditItem(name, CommonHelper.FIELD_START, "13:00").execute(service, project, history);
        event = service.viewSpecificEvent(name);
        assertEquals(event.getStartDateTimeString(), "19 Jan 2015 13:00");
        
        // test can edit start date and time
        new CommandEditItem(name, CommonHelper.FIELD_START, "20 Sep 2015 13:00").execute(service, project, history);
        event = service.viewSpecificEvent(name);
        assertEquals(event.getStartDateTimeString(), "20 Sep 2015 13:00");
        
        // test can edit end date
        new CommandEditItem(name, CommonHelper.FIELD_END, "19 Dec 2015").execute(service, project, history);
        event = service.viewSpecificEvent(name);
        assertEquals(event.getEndDateTimeString(), "19 Dec 2015 23:00");
        
        // test can edit end time
        new CommandEditItem(name, CommonHelper.FIELD_END, "13:00").execute(service, project, history);
        event = service.viewSpecificEvent(name);
        assertEquals(event.getEndDateTimeString(), "19 Dec 2015 13:00");
        
        // test can edit end date and time
        new CommandEditItem(name, CommonHelper.FIELD_END, "20 Nov 2015 15:00").execute(service, project, history);
        event = service.viewSpecificEvent(name);
        assertEquals(event.getEndDateTimeString(), "20 Nov 2015 15:00");
        
        // test can edit name
        new CommandEditItem(name, CommonHelper.FIELD_NAME, "eat meh").execute(service, project, history);
    ...
    public void testCanEditTodo() throws Exception {
        ...
        new CommandEditItem(name, CommonHelper.FIELD_DUE, "19 Dec 2015").execute(service, project, history);
        todo = service.viewSpecificTask(name);
        assertEquals(todo.getDeadlineDateTimeString(), "19 Dec 2015 10:00");
        
        // test can edit due time
        new CommandEditItem(name, CommonHelper.FIELD_DUE, "13:00").execute(service, project, history);
        todo = service.viewSpecificTask(name);
        assertEquals(todo.getDeadlineDateTimeString(), "19 Dec 2015 13:00");
        
        // test can edit due date and time
        new CommandEditItem(name, CommonHelper.FIELD_DUE, "20 Nov 2015 13:00").execute(service, project, history);
        todo = service.viewSpecificTask(name);
        assertEquals(todo.getDeadlineDateTimeString(), "20 Nov 2015 13:00");
        
        // test can edit name
        new CommandEditItem(name, CommonHelper.FIELD_NAME, "eat meh meh").execute(service, project, history);
